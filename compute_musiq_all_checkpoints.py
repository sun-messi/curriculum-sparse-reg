#!/usr/bin/env python
"""
Compute MUSIQ scores for all checkpoints across different training methods.

MUSIQ (Multi-Scale Image Quality Transformer) is a Transformer-based
no-reference metric. Higher scores = better quality.
...
def compute_musiq_folder(folder_path, device='cuda' if torch.cuda.is_available() else 'cpu', batch_size=32):
    """
    Compute average MUSIQ score for all images in a folder.
    ...
    Returns:
        Average MUSIQ score (higher is better)
    """
    ...
    pbar = tqdm(total=len(image_files), desc="  Computing MUSIQ", leave=False)
    ...
    for batch_idx in range(num_batches):
        ...
        if len(batch_tensors) > 0:
            ...
            with torch.no_grad():
                batch_scores = musiq_metric(batch)
    ...
    if len(scores) == 0:
        raise ValueError(f"Failed to compute MUSIQ for any images in {folder_path}")
...
def compute_all_musiq():
    """Compute MUSIQ for all checkpoints."""
    ...
    for method_name, base_dir in SAMPLE_DIRS.items():
        ...
        for checkpoint in CHECKPOINTS:
            ...
            try:
                score = compute_musiq_folder(checkpoint_dir, device=device, batch_size=16)
                print(f"  ✓ {method_name} {checkpoint}: {score:.4f}\n")
                results[method_name][checkpoint] = score
            except Exception as e:
                import traceback
                print(f"  ✗ Error computing MUSIQ for {checkpoint}: {e}")
                traceback.print_exc()
                results[method_name][checkpoint] = None
...
def format_output(results):
    ...
    # Header
    lines.append("=" * 80)
    lines.append("U-ViT CelebA-64 MUSIQ Comparison (All Checkpoints)")
    lines.append(f"Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append("Dataset: celeba")
    lines.append("Images per checkpoint: ~5000 generated images")
    lines.append("Evaluation Metric: MUSIQ (Multi-Scale Image Quality Transformer, higher is better)")
    lines.append("=" * 80)
    lines.append("")
    ...
    # Find best MUSIQ for each method
    lines.append("BEST MUSIQ (higher is better):")
    for method_name in method_names:
    ...
    lines.append("")
    lines.append("Note: MUSIQ (Multi-Scale Image Quality Transformer) is a Vision Transformer-based")
    lines.append("      no-reference metric designed to handle images of various resolutions.")
    lines.append("      Higher scores indicate better perceptual quality.")
    lines.append("")
    lines.append("=" * 80)
...
def main():
    """Main function."""
    print("=" * 80)
    print("MUSIQ Comparison Script - All Checkpoints")
    print("=" * 80)
    print("\nThis will compute MUSIQ scores for all checkpoints (10k-190k).")
    print("MUSIQ is a Transformer-based no-reference metric for perceptual quality.\n")

    # Compute MUSIQ scores
    results = compute_all_musiq()
    ...
    # Save to file
    output_file = "uvit_celeba64_musiq_comparison.txt"
